...
function OurProducts() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${BASEURL}/items/products?filter[name][_contains]=${search}&fields=*.product_category_id.name,images.images_id.*.image_category_id.name`
      )
      .then((response) => {
        setProducts(response.data?.data);
      })
    ...
    <Layout>
      <div className="px-8 md:px-16 py-5 md:py-10 container mx-auto">
        <div className="md:flex justify-between ">
          <p className="text-2xl sm:block md:text-3xl md:flex justify-center px-10 font-bold mb-4 sm:mb-0 bg pt-2">
            Our Products
          </p>
      ...
      <div className="container md:flex md:even:flex-row-reverse gap-60 p-8 md:py-24 md:px-32">
        <div className="pb-10 sm:pb-0">
          <Image
            ...
            className=" w-[500px] h-[300px] object-contain md:-mt-10 "
            alt="yarsa tech product "
          />
        </div>
        <div className="">
          <p className="text-3xl font-bold">{product.name} </p>
          <p className="text-lg dark:text-blue-600 text-custom-blue font-semibold ">
            {product.main_features}
          </p>
          <p className="py-4 ">{product.description}</p>
          <div className="md:flex  flex md:gap-6 gap-3  ">
            <button
              className="button rounded-md border  text-base text-white bg-custom-blue md:py-3 md:px-5 py-2 px-3  tracking-wider mt-5"
              onClick={() => {
                navigate.push({
                  pathname: "/productDetails",
                  query: { id: product.id },
